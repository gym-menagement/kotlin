...
CREATE TABLE `notificationsetting_tb` (
    ...
    `ns_membershipexpiry` INT(11) NOT NULL DEFAULT 0 COMMENT '이용권 만료 알림 (0:활성화, 1:비활성화)',
    `ns_membershipnear` INT(11) NOT NULL DEFAULT 0 COMMENT '이용권 만료 임박 알림 D-7, D-3, D-1 (0:활성화, 1:비활성화)',
    `ns_attendanceenc` INT(11) NOT NULL DEFAULT 0 COMMENT '출석 독려 알림 (0:활성화, 1:비활성화)',
    `ns_gymannounce` INT(11) NOT NULL DEFAULT 0 COMMENT '체육관 공지사항 알림 (0:활성화, 1:비활성화)',
    `ns_systemnotice` INT(11) NOT NULL DEFAULT 0 COMMENT '시스템 공지 알림 (0:활성화, 1:비활성화)',
    `ns_paymentconfirm` INT(11) NOT NULL DEFAULT 0 COMMENT '결제 확인 알림 (0:활성화, 1:비활성화)',
    `ns_pauseexpiry` INT(11) NOT NULL DEFAULT 0 COMMENT '일시정지 만료 알림 (0:활성화, 1:비활성화)',
    `ns_weeklygoal` INT(11) NOT NULL DEFAULT 0 COMMENT '주간 목표 달성 알림 (0:활성화, 1:비활성화)',
    `ns_personalrecord` INT(11) NOT NULL DEFAULT 0 COMMENT '개인 기록 갱신 알림 (0:활성화, 1:비활성화)',
    `ns_quietenabled` INT(11) NOT NULL DEFAULT 1 COMMENT '방해 금지 시간 사용 여부 (0:활성화, 1:비활성화)',
    `ns_quietstart` TIME NULL COMMENT '방해 금지 시작 시간 (예: 22:00:00)',
    `ns_quietend` TIME NULL COMMENT '방해 금지 종료 시간 (예: 08:00:00)',
