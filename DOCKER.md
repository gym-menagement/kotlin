...
```bash
# 서버에서 직접 실행
docker pull gowoobro/gymspring:latest
...
```bash
# 8004 포트를 사용하는 프로세스 확인
lsof -i :8004

# 또는 다른 포트로 실행
docker run -d --name gymspring-app -p 9004:8004 gowoobro/gymspring:latest
```
...
```bash
docker stats gymspring-app
```

### 컨테이너 상세 정보

```bash
docker inspect gymspring-app
```

### 실행 중인 프로세스

```bash
docker top gymspring-app
```
...
ssh user@server
docker pull gowoobro/gymspring:latest
docker stop gymspring-app
docker rm gymspring-app
docker run -d --name gymspring-app -p 8004:8004 gowoobro/gymspring:latest
```
...
```bash
# 특정 버전으로 롤백
docker run -d --name gymspring-app -p 8004:8004 gowoobro/gymspring:v1.0.0
```
