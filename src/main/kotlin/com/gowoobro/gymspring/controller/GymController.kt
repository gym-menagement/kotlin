...
class GymController(
    ...
    fun getGyms(
        ...
        var results = if (name != null || address != null || tel != null || user != null || startdate != null || enddate != null || false) {
            var filtered = gymService.findAll(0, Int.MAX_VALUE).content
            if (name != null) {
            ...
            gymService.findAll(0, Int.MAX_VALUE).content
        }
    ...
    fun patchGym(
        @PathVariable id: Long,
        @RequestBody request: GymPatchRequest
    ): ResponseEntity<GymResponse> {
        val patchRequest = request.copy(id = id)
        val res = gymService.patch(patchRequest)
        return if (res != null) {
            ResponseEntity.ok(toResponse(res))
        } else {
            ResponseEntity.notFound().build()
