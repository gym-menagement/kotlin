...
interface NotificationhistoryRepository : JpaRepository<Notificationhistory, Long> {
    ...
    // 커스텀 쿼리 메서드
    @Query("SELECT n FROM Notificationhistory n WHERE n.receiverId = :receiverId ORDER BY n.sentdate DESC")
    fun findByReceiverOrderBySentdateDesc(receiverId: Long, pageable: Pageable): Page<Notificationhistory>

    @Query("SELECT n FROM Notificationhistory n WHERE n.gymId = :gymId ORDER BY n.sentdate DESC")
    ...
    fun findByTypeOrderBySentdateDesc(type: Type, pageable: Pageable): Page<Notificationhistory>

    @Query("SELECT n FROM Notificationhistory n WHERE n.receiverId = :receiverId AND n.type = :type ORDER BY n.sentdate DESC")
    fun findByReceiverAndTypeOrderBySentdateDesc(receiverId: Long, type: Type, pageable: Pageable): Page<Notificationhistory>
}
